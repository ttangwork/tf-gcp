...
resource "google_compute_instance" "bastion" {
  ...
  metadata = {
    ...
    startup-script = <<-EOT
      ...
      apt-get update
      apt-get install -y git make kubectl kubectx google-cloud-cli-gke-gcloud-auth-plugin
      dpkg -s git make kubectl kubectx google-cloud-cli-gke-gcloud-auth-plugin > /dev/null 2>&1
